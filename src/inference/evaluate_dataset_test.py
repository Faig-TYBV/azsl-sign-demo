...
def main():
    # Paths
    metadata_path = Path(r"c:\Users\ASUS\Desktop\azsl-word-recognition\outputs\dataset_split.json")
    checkpoint_path = Path(r"c:\Users\ASUS\Desktop\azsl-word-recognition\outputs\checkpoints\gru_temporal_pool_best.pt")
    output_dir = Path(r"c:\Users\ASUS\Desktop\azsl-word-recognition\outputs\inference_test")
    ...
    test_feature_paths = test_feature_paths[:10]
    ...
    fieldnames = ['video_path', 'true_class', 'predicted_class', 'correct', 'confidence', 'top5_predictions', 'num_frames', 'valid_frames']
    ...
    report = {
        ...
        "number_correct": int(sum(all_true == all_pred)),
        "number_incorrect": int(len(all_true) - sum(all_true == all_pred)),
